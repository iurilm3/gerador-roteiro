          ...
          {OBJETIVOS.map((obj) => {
            ...
            return (
              <button key={obj.id} onClick={() => setObjetivo(obj.id)}
                ...
                <p className={`font-semibold text-sm mb-1 ${ativo ? "text-violet-700 dark:text-violet-300" : "text-zinc-700 dark:text-zinc-300"}`}>
                  {obj.label}
                </p>
                <p className="text-xs leading-relaxed text-zinc-400 dark:text-zinc-500">{obj.descricao}</p>
              </button>
            );
          })}
        </div>
      ...
      <div className="mb-7">
        ...
        <div className="grid grid-cols-2 gap-3">
          {TIPOS_TRAFEGO.map((t) => {
            ...
            return (
              <button key={t.id} onClick={() => setTipoTrafego(t.id)}
                className={`text-left p-4 rounded-xl border transition-all ${ativo ? cardAtivo : cardInativo}`}
                style={{ minHeight: "80px" }}>
                <p className={`font-semibold text-sm mb-1 ${ativo ? "text-violet-700 dark:text-violet-300" : "text-zinc-700 dark:text-zinc-300"}`}>
                  {t.label}
                </p>
                <p className="text-xs leading-relaxed text-zinc-400 dark:text-zinc-500">{t.descricao}</p>
              </button>
            );
          })}
        </div>
          ...
          {FORMATOS.map((f) => {
            ...
            return (
              <button key={f.id} onClick={() => setFormato(f.id)}
                ...
                <p className="text-xs leading-relaxed text-zinc-400 dark:text-zinc-500">{f.descricao}</p>
              </button>
            );
          })}
        </div>
